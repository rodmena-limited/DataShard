"""
LEGACY (pre-0.10) snake_case metadata JSON <-> TableMetadata. Read-only in 0.10+: the
migration tool (#89) and the legacy-layout detector use it; nothing writes this form.
"""
...
def legacy_metadata_to_dict(metadata: TableMetadata) -> Dict[str, Any]:
    """Legacy snake_case form (kept for the migration fixtures / tests only)."""
    return {
...
def legacy_dict_to_metadata(metadata_dict: Dict[str, Any]) -> TableMetadata:
    """Parse the legacy snake_case form."""
